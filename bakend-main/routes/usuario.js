...
const { emailExiste, esRoleValido, existeUsuarioPorId, esTipoValido } = require('../helpers/db-validators');
...
router.post('/agregar', [
    ...
    check('dpi','El dpi es obligatorio para el post').not().isEmpty(),
    check('celular','El numero de celular es obligatorio para el post').not().isEmpty(),
    ...
    check('tipoCuenta', 'El tipo de cuenta es obligatorio para el post').not().isEmpty(),
...
router.put('/editar/:id',[
    ...
    check('dpi').custom((value, { req }) => {
        if (value !== undefined) {
          return Promise.reject('El campo DPI no se puede editar');
        }
        return true;
      }),
    check('celular','El numero de celular es obligatorio para el post').not().isEmpty(),
